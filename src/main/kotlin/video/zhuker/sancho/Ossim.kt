...
class ossimApplanixEcefModel(
    ...
    val theRoll: Double,
    val thePitch: Double,
    val theHeading: Double,
    principalPoint: ossimDpt,
    val theFocalLength: Double,
    pixelSize: ossimDpt
) {
    val theImageSize = ossimIpt()
    val thePixelSize = pixelSize.copy()
    val theImageClipRect = imageRect
    val theRefImgPt: ossimDpt = theImageClipRect.midPoint()
    val thePrincipalPoint = principalPoint.copy()
    val theLensDistortion = ossimMeanRadialLensDistortion()
    var theEcefPlatformPosition: ossimEcefPoint = ossimEcefPoint(platformPosition)
    var theAdjEcefPlatformPosition: ossimEcefPoint = ossimEcefPoint(platformPosition);

    var theCompositeMatrix = SimpleMatrix.diag(0.0, 0.0, 0.0, 0.0);
    private var theCompositeMatrixInverse = SimpleMatrix.diag(0.0)

    var theBoundGndPolygon: ossimPolygon = ossimPolygon()
    private var theExtrapolateImageFlag: Boolean = false
    private var theExtrapolateGroundFlag: Boolean = false
    var theGSD = ossimDpt()// meters
    var theMeanGSD = 0.0 // meters
